...
class KNearestNeighbor(object):
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in range(num_test):
      ...
      #print(np.sum(subtr * subtr, axis=1))
      #print(np.sum(np.power(np.subtract(X[i], self.X_train), 2), axis=1))
      #summ = np.sum(pwr, axis=1)
      subtr = X[i, :] - self.X_train
      dists[i, :] = np.sqrt(np.sum(subtr * subtr, axis=1))
      #print(X[i])
      #print(X[i, :])
      #######################################################################
      #                         КОНЕЦ ВАШЕГО КОДА                           #
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    x1 = np.sum(X * X, axis=1).reshape(num_test, 1)
    x2 = np.sum(self.X_train * self.X_train, axis=1).reshape(1, num_train)
    dists = np.sqrt(x1 + x2 - 2 * np.dot(X, self.X_train.T))
    #########################################################################
    #                         КОНЕЦ ВАШЕГО КОДА                             #
    #########################################################################
    return dists
